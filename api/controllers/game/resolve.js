    ...
    .then(function changeAndSave(values) {
      ...
      return sails.getDatastore().transaction((db) => {
          ...
          switch (game.oneOff.rank) {
            ...
            case 5:
              ...
              updatePromises = [
                ...
                Game.removeFromCollection(game.id, 'deck')
                  ...
                  .usingConnection(db),
                User.addToCollection(player.id, 'hand').members(cardsToDraw).usingConnection(db),
              ];
            ...
            case 6:
              ...
              updatePromises = [
                ...
                Card.replaceCollection(allPoints, 'attachments').members([]).usingConnection(db),
        ...
        updatePromises = [
          ...
          Game.addToCollection(game.id, 'scrap').members(cardsToScrap).usingConnection(db),
          // Clear twos
          Game.replaceCollection(game.id, 'twos').members([]).usingConnection(db),
          // Update opponent, as specified
          User.updateOne(opponent.id).set(opponentUpdates).usingConnection(db),
        ];
